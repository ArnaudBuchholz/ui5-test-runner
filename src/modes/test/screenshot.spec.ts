...
describe('handlePendingScreenshot', () => {
  it('does nothing when screenshot option is disabled', async () => {
    const config = { ...BASE_CONFIG, screenshot: false } as unknown as Configuration;
    const { handlePendingScreenshot } = makeScreenshotHandlers(config);
    const page = makePage();
    await handlePendingScreenshot(page, PAGE_ID);
  ...
  it('does nothing when pendingScreenshot is false', async () => {
    ...
    const page = makePage({ eval: vi.fn().mockResolvedValue(QUnit_STATE_NO_PENDING) });
    await handlePendingScreenshot(page, PAGE_ID);
  ...
  it('takes screenshot to the agent-supplied filename and clears the flag', async () => {
    ...
    const { handlePendingScreenshot } = makeScreenshotHandlers(BASE_CONFIG);
    const page = makePage({
      eval: vi.fn().mockResolvedValueOnce(QUnit_STATE_WITH_PENDING(FILENAME)).mockResolvedValue(undefined)
    });
    await handlePendingScreenshot(page, PAGE_ID);
