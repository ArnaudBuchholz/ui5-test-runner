...
vi.mocked(Process.spawn).mockImplementation((command, arguments_) => {
  ...
  // eslint-disable-next-line unicorn/prefer-ternary -- or code becomes undreadable
  if (arguments_ && arguments_.includes('--global')) {
    return makeProcess('/global/root');
  }
  return makeProcess('/local/root');
});
...
describe('checkIfLatestVersion', () => {
  const mockResolveLocal = () => {
    vi.mocked(Module.findPackageJSON).mockReturnValue('/local/root/some-module/package.json');
  };
  ...
  it('swallows errors and reports them via logger.error', async () => {
    vi.mocked(Module.findPackageJSON).mockReturnValue(undefined);
...
describe('resolvePackageDir', () => {
  it('returns the package dir when found in local root', async () => {
    vi.mocked(Module.findPackageJSON).mockReturnValueOnce('/local/root/some-module/package.json');
  ...
  it('returns the package dir when found in global root', async () => {
    vi.mocked(Module.findPackageJSON)
      .mockReturnValueOnce(undefined)
      .mockReturnValueOnce('/global/root/some-module/package.json');
  ...
  it('returns the package dir when found in alternateNpmPath', async () => {
    vi.mocked(Module.findPackageJSON)
      .mockReturnValueOnce(undefined)
      .mockReturnValueOnce(undefined)
      .mockReturnValueOnce('/alternate/path/some-module/package.json');
  ...
  it('returns the package dir when found in npmInstallPrefix', async () => {
    vi.mocked(Module.findPackageJSON)
      .mockReturnValueOnce(undefined)
      .mockReturnValueOnce(undefined)
      .mockReturnValueOnce('/prefix/path/node_modules/some-module/package.json');
