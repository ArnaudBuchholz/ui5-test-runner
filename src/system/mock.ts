import { vi } from 'vitest';
import type { WriteStream } from 'node:fs';
import type { BroadcastChannel, Worker } from 'node:worker_threads';
import type { Gzip } from 'node:zlib';

const mockStaticMethods = <T extends object>(actual: T): T => {
  const mocked = { ...actual };
  for (const name in Object.keys(mocked)) {
    const property = name as keyof T;
    const value = mocked[property];
    if (typeof value === 'function') {
      mocked[property] = vi.fn();
    }
  }
  return mocked;
}

vi.mock(import('./Exit.js'), async (importActual) => {
  const mocked = mockStaticMethods(await importActual());
  return mocked;
});

vi.mock(import('./FileSystem.js'), async (importActual) => {
  const writeStream = {} as WriteStream;
  const mocked = mockStaticMethods(await importActual());
  vi.mocked(mocked.FileSystem.createWriteStream).mockReturnValue(writeStream);
  return mocked;
});

vi.mock(import('./Http.js'), async (importActual) => {
  const mocked = mockStaticMethods(await importActual());
  return mocked;
});

vi.mock(import('./Process.js'), async (importActual) => {
  const mocked = mockStaticMethods(await importActual());
  return mocked;
});

vi.mock(import('./Terminal.js'), async (importActual) => {
  const mocked = mockStaticMethods(await importActual());
  return mocked;
});

vi.mock(import('./Thread.js'), async (importActual) => {
  const mocked = mockStaticMethods(await importActual());
  const channel = {
    postMessage: vi.fn().mockImplementation((data: unknown) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-argument
      channel.onmessage?.({ data } as any);
    }),
    onmessage: undefined as ((data: unknown) => void) | undefined,
    close: vi.fn()
  } as unknown as BroadcastChannel;
  const eventTarget = new EventTarget();
  const worker = eventTarget as unknown as Worker;
  Object.assign(worker, {
    on: eventTarget.addEventListener.bind(eventTarget),
    postMessage: vi.fn()
  });
  vi.mocked(mocked.Thread.createBroadcastChannel).mockReturnValue(channel);
  vi.mocked(mocked.Thread.createWorker).mockReturnValue(worker);
  return mocked;
});

vi.mock(import('./ZLib.js'), async (importActual) => {
  const mocked = mockStaticMethods(await importActual());
  const gzipStream = {
    pipe: vi.fn(),
    write: vi.fn(),
    end: vi.fn()
  } as unknown as Gzip;
  vi.mocked(mocked.ZLib.createGzip).mockReturnValue(gzipStream);
  return mocked;
});
