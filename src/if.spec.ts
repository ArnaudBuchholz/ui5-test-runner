...
describe('isIfEvaluatedAsTrue()', () => {
  ...
  describe('context variables', () => {
    it('exposes NODE_MAJOR_VERSION as a number', async () => {
      const shouldExecute = await isIfEvaluatedAsTrue(makeConfig('NODE_MAJOR_VERSION === 24'));
      expect(shouldExecute).toBe(true);
    });

    it('exposes NODE_VERSIOM (full version string)', async () => {
      const shouldExecute = await isIfEvaluatedAsTrue(makeConfig('NODE_VERSIOM === "v24.18.0"'));
      expect(shouldExecute).toBe(true);
    });

    it('exposes UI5TR_NAME', async () => {
      const shouldExecute = await isIfEvaluatedAsTrue(makeConfig('UI5TR_NAME === "ui5-test-runner"'));
      expect(shouldExecute).toBe(true);
    });

    it('exposes UI5TR_VERSION', async () => {
      const shouldExecute = await isIfEvaluatedAsTrue(makeConfig('UI5TR_VERSION === "6.0.0"'));
      expect(shouldExecute).toBe(true);
    });

    it('exposes Host.env variables', async () => {
      Object.assign(Host, { env: { MY_VAR: 'hello' } });
      const shouldExecute = await isIfEvaluatedAsTrue(makeConfig('MY_VAR === "hello"'));
      expect(shouldExecute).toBe(true);
    });

    it('exposes compareVersions helper', async () => {
      const shouldExecute = await isIfEvaluatedAsTrue(makeConfig('compareVersions(UI5TR_VERSION, "5.0.0") === 1'));
      expect(shouldExecute).toBe(true);
    });
  });
  ...
  describe('expression evaluation', () => {
    it('returns true when condition evaluates to truthy', async () => {
      const shouldExecute = await isIfEvaluatedAsTrue(makeConfig('1 === 1'));
      expect(shouldExecute).toBe(true);
    });

    it('returns false when condition evaluates to falsy', async () => {
      const shouldExecute = await isIfEvaluatedAsTrue(makeConfig('1 === 2'));
      expect(shouldExecute).toBe(false);
    });

    it('returns false when condition evaluates to 0', async () => {
      const shouldExecute = await isIfEvaluatedAsTrue(makeConfig('0'));
      expect(shouldExecute).toBe(false);
    });
